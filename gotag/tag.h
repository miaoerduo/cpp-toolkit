...
class any {
public:
    any(void* data, const std::string& tag, bool required, const std::string& help)
        : data_(data), tag_(tag), help_(help) {}
    ~any() = default;
...
template <typename T>
class scalar : public any {
public:
    scalar(void* data, const std::string& tag, bool required, const std::string& help)
        : any(data, tag, required, help) {}
};

template <typename T>
class pointer : public any {
public:
    pointer(void* data, const std::string& tag, bool required, const std::string& help)
        : any(data, tag, required, help) {}
};
...
public:
    vector(void* data, const std::string& tag, bool required, const std::string& help)
        : any(data, tag, required, help) {}
};
...
public:
    map(void* data, const std::string& tag, bool required, const std::string& help) : any(data, tag, required, help) {}
};
...
public:
    ...
    std::list<std::shared_ptr<::med::field::any>> field_list_;
    std::unordered_map<std::string, std::vector<std::shared_ptr<::med::field::any>>> field_map_;
};

}  // namespace med

#define __GET_6TH_ARG__(arg0, arg1, arg2, arg3, arg4, func, ...) func
#define __GET_7TH_ARG__(arg0, arg1, arg2, arg3, arg4, arg5, func, ...) func
#define __GET_8TH_ARG__(arg0, arg1, arg2, arg3, arg4, arg5, arg6, func, ...) func

#define __DEFINE_VAL__(v, type, tag, required, help)                                         \
    type v = [=]() {                                                                         \
        auto f = std::make_shared<::med::field::scalar<type>>(&this->v, (tag), (required), (help)); \
        this->field_list_.push_back(f);                                                      \
        this->field_map_[(tag)].push_back(f);                                                \
        return type{};                                                                       \
    }()
...
#define __DEFINE_VAL_WITH_DEFAULT__(v, type, tag, default_val, help)                    \
    type v = [=]() {                                                                    \
        auto f = std::make_shared<::med::field::scalar<type>>(&this->v, (tag), false, (help)); \
        this->field_list_.push_back(f);                                                 \
        this->field_map_[(tag)].push_back(f);                                           \
        return (default_val);                                                           \
    }()
...
#define DEFINE_VAL_D(...)                                                      \
    __GET_7TH_ARG__(, ##__VA_ARGS__, __DEFINE_VAL_WITH_DEFAULT__(__VA_ARGS__), \
                    __DEFINE_VAL_WITH_DEFAULT__(__VA_ARGS__, ""))

#define __DEFINE_VECTOR__(v, type, tag, required, help)                                      \
    std::vector<type> v = [=]() {                                                            \
        auto f = std::make_shared<::med::field::vector<type>>(&this->v, (tag), (required), (help)); \
        this->field_list_.push_back(f);                                                      \
        this->field_map_[(tag)].push_back(f);                                                \
...
#define DEFINE_VECTOR(...)                                                                                 \
    __GET_7TH_ARG__(, ##__VA_ARGS__, __DEFINE_VECTOR__(__VA_ARGS__), __DEFINE_VECTOR__(##__VA_ARGS__, ""), \
                    __DEFINE_VECTOR__(##__VA_ARGS__, false, ""))

#define __DEFINE_POINTER__(v, type, tag, required, help)                                      \
    std::shared_ptr<type> v = [=]() {                                                         \
        auto f = std::make_shared<::med::field::pointer<type>>(&this->v, (tag), (required), (help)); \
        this->field_list_.push_back(f);                                                       \
        this->field_map_[(tag)].push_back(f);                                                 \
        return nullptr;                                                                       \
    }()

#define DEFINE_POINTER(...)                                                                                \
    __GET_7TH_ARG__(, ##__VA_ARGS__, __DEFINE_POINTER__(__VA_ARGS__), __DEFINE_POINTER__(__VA_ARGS__, ""), \
                    __DEFINE_POINTER__(__VA_ARGS__, false, ""))

#define __DEFINE_MAP__(v, type, tag, required, help)                                      \
    std::unordered_map<std::string, type> v = [=]() {                                     \
        auto f = std::make_shared<::med::field::map<type>>(&this->v, (tag), (required), (help)); \
        this->field_list_.push_back(f);                                                   \
        this->field_map_[(tag)].push_back(f);                                             \
        return nullptr;                                                                   \
    }()
...
#define __DEFINE_MAP(...)                                                                            \
    __GET_7TH_ARG__(, ##__VA_ARGS__, __DEFINE_MAP__(__VA_ARGS__), __DEFINE_MAP__(##__VA_ARGS__, ""), \
                    __DEFINE_MAP__(##__VA_ARGS__, false, ""))
...
class LinkedList : public med::Tag {
public:
    DEFINE_VAL(val_, std::string, "val", true);
    DEFINE_POINTER(next_, LinkedList, "next");
};

class MyData : public med::Tag {
public:
    DEFINE_VAL(string_, std::string, "string", true, "string data");
    DEFINE_VAL(int32_, std::string, "int32", true, "int32 data");
    DEFINE_POINTER(linked_list_, LinkedList, "linked_list");
};

#include <iostream>
int main() {
    MyData d;
    std::cout << d.string_ << " " << d.field_list_.size() << std::endl;
}
...
// /**
//  * @brief Base Parser for Any Data
//  *
//  * @tparam DocType
//  */
...
//     virtual bool Init(const std::string& data) = 0;
//     virtual bool Init(const DocType& data) = 0;
...
//     virtual Parser GetMember(const std::string& key) = 0;

//     virtual bool IsArray() = 0;
//     virtual size_t GetArraySize() = 0;
//     virtual Parser GetArrayItem(size_t idx) = 0;
// };

// template <typename DocType>
// class BaseField {
// public:
//     BaseField(void* data, const std::string& name, const std::string& desc) : data_(data), name_(name), desc_(desc) {}
//     virtual bool Parse(Parser<DocType>& parser) = 0;
...
//         Parser<T> parser;
//         if (!parser.Init(str)) {
//             std::cerr << "parser init fail" << std::endl;
//             return false;
//         }

//     }
//     virtual bool Parse(const T& data) {
